namespace Skyline.DataMiner.DeveloperCommunityLibrary.YLE.Integrations.Eurovision
{
	...
	public class EurovisionIntegration : Integration
	{
		...
		public EurovisionIntegration(Helpers helpers, OrderManagerElement orderManagerElement) : base(helpers, orderManagerElement)
		{
		}
		...
		{
			Log(nameof(HandleUpdate), "Handling update for Transmission Number: " + id);

			if (!TryParseSynopsis(integrationData, out synopsis synopsis))
			{
				Log(nameof(HandleUpdate), $"Unable to retrieve Synopsis -> no action performed");
				SendResponseToOrderManagerElement(id, UpdateStatus.Failed, "HandleUpdate|No action performed|Unable to parse synopsis");
				return;
			}

			if (!String.Equals(id, synopsis.transmissionNo))
			{
				Log(nameof(HandleUpdate), $"Integration number {id} doesn't match with Transmission Number in synopsis {synopsis.transmissionNo}");
				SendResponseToOrderManagerElement(id, UpdateStatus.Failed, $"HandleUpdate|No action performed|Integration number {id} doesn't match with Transmission Number in synopsis {synopsis.transmissionNo}");
				return;
			}

			// No order or note needed for fixed EBU news channels
			if (Array.Exists(synopsis.technicalSystems, x => x.IsNewsTechnicalSystem))
			{
				Log(nameof(HandleUpdate), $"Synopsis defines Fixed News Order -> no action performed");
				SendResponseToOrderManagerElement(id, UpdateStatus.OK, "HandleUpdate|No action performed|Fixed news");
				return;
			}

			Order order = GenerateEbuOrder(synopsis, out var ex);
			if (order != null)
			{
				Log(nameof(HandleUpdate), $"Adding or updating order");

				AddOrUpdateEurovisionOrder(id, order, synopsis);
				TryDeleteSynopsisNote(synopsis);
			}
			else
			{
				Log(nameof(HandleUpdate), $"Adding or updating note");

				var note = GenerateEbuNote(synopsis);
				AddOrUpdateEurovisionNote(note, ex);
				DeleteOrdersForSynopsis(synopsis);
			}
		}

		private void DeleteOrdersForSynopsis(synopsis synopsis)
		{
			var orders = helpers.OrderManager.GetEurovisionOrders(synopsis.transmissionNo);
			orders.AddRange(GetManuallyBookedOrders(synopsis.transmissionNo));

			foreach (var order in orders)
			{
				helpers.OrderManager.DeleteOrder(order);
				Log(nameof(DeleteOrdersForSynopsis), $"Deleted {order.Name} with ID: {order.Id}");
			}
		}

		private bool TryParseSynopsis(string serializedSynopsis, out synopsis synopsis)
		{
			synopsis = null;
			try
			{
				synopsis = synopsis.Deserialize(serializedSynopsis);
			}
			catch (Exception e)
			{
				Log(nameof(TryParseSynopsis), $"Unable to parse synopsis: {e}");
				return false;
			}
		...
		private void TryDeleteSynopsisNote(synopsis synopsis)
		{
			if (helpers.NoteManager.TryGetEurovisionNote(synopsis.transmissionNo, out var synopsisNote))
			{
				Log(nameof(TryDeleteSynopsisNote), $"Found existing Eurovision note with Transmission Number: {synopsis.transmissionNo} and title: {synopsisNote?.Title}");

				// A note can exist for a previous version of the synopsis, delete it in case an order was created
				helpers.NoteManager.DeleteNote(synopsisNote);
			}
		}
		...
		private List<CustomerService> GenerateReceptions(synopsis synopsis)
		{
			...
			{
				Log(nameof(GenerateReceptions), $"Checking destination {destination.destinationId}");
				var contract = GetValidContractForDestination(destination, synopsis.StartDateTime, synopsis.EndDateTime);
				if (contract == null)
				{
					Log(nameof(GenerateReceptions), $"No valid contract found for destination {destination.destinationId}");
					continue;
				}
				...
				{
					bool receptionAlreadyExists = customerServices.Any(cs => DoServicesMatch(cs.Service, reception, IgnoredItems.None));
					if (!receptionAlreadyExists) customerServices.Add(new CustomerService(contract, reception));
				}
			}

			Log(nameof(GenerateReceptions), $"Generated {customerServices.Count} possible Reception Services: {String.Join(", ", customerServices.Select(cs => "[Service: " + cs.Service.Definition.Name + ", Contract: " + cs.Contract.Name + "]"))}");
		...
		{
			Log(nameof(GenerateReceptionsForDestination), $"Creating reception service for Destination: {destination.destinationId}");

			var technicalSystem = synopsis.technicalSystems.FirstOrDefault(x => !String.IsNullOrEmpty(x?.technicalSystemId) && String.Equals(destination.technicalSystemId, x.technicalSystemId));
			if (technicalSystem == null)
			{
				Log(nameof(GenerateReceptionsForDestination), $"No technical system linked to Destination: {destination.destinationId}");
				return new Service[0];
			}

			return GenerateReceptionsForTechnicalSystem(synopsis, destination, technicalSystem);
		}
		...
		private Contract GetValidContractForDestination(synopsisDestination destination, DateTime startTime, DateTime endTime)
		{
			...
			{
				Log(nameof(GetValidContractForDestination), $"Unable to map organization code {destination.organizationCode}");
				return null; // No supported organisation
			}
			...
			{
				Log(nameof(GetValidContractForDestination), $"Contract manager does not contain an entry for {OrganizationCodeMapping[organizationCode]}");
				return null;
			}
			...
			{
				Log(nameof(GetValidContractForDestination), $"No response received from Contract Manager for company {companyName}");
				return null;
			}
			...
			{
				Log(nameof(GetValidContractForDestination), $"No base contract defined for company {companyName}");
				return null;
			}

			if (baseContract.Start > startTime || baseContract.End < endTime)
			{
				Log(nameof(GetValidContractForDestination), $"Contract {baseContract.Name} is not valid from {startTime} to {endTime}");
				return null;
			}
		...
		private IEnumerable<Service> GenerateReceptionsForTechnicalSystem(synopsis synopsis, synopsisDestination destination, synopsisTechnicalSystem technicalSystem)
		{
			...
			{
				Log(nameof(GenerateReceptionsForTechnicalSystem), $"Generated Sat RX service");
				receptions.Add(satelliteReception);
			}

			if (synopsis.routings == null)
			{
				Log(nameof(GenerateReceptionsForTechnicalSystem), $"Synopsis doesn't contain any routings");
				return receptions;
			}
			...
			if (containsAceToYleRoutings && TryGenerateFixedLineEbuRxService(synopsis, technicalSystem, acesRoute, out Service fixedLineEbuAcesRxService))
			{
				Log(nameof(GenerateReceptionsForTechnicalSystem), $"Generated ACE TO YLE Fixed Line EBU Rx Service");
				receptions.Add(fixedLineEbuAcesRxService);
			}
			...
			if (containsNordicRingRoutings && TryGenerateFixedLineEbuRxService(synopsis, technicalSystem, nordicRingRoute, out Service fixedLineEbuNordicRingRxService))
			{
				Log(nameof(GenerateReceptionsForTechnicalSystem), $"Generated Nordic Ring Fixed Line EBU Rx Service");
				receptions.Add(fixedLineEbuNordicRingRxService);
			}
			...
			if (containsFiberPorts && !containsAceToYleRoutings && !containsNordicRingRoutings && TryGenerateFullCapacityFiberRxService(synopsis, technicalSystem, out Service fullCapacityFiberRxService))
			{
				Log(nameof(GenerateReceptionsForTechnicalSystem), $"Generated Fiber RX Full Capacity Service");
				receptions.Add(fullCapacityFiberRxService);
			}
		...
		private Service GenerateSatelliteRxService(synopsis synopsis, synopsisDestination destination, synopsisTechnicalSystem technicalSystem)
		{
			...
			FunctionResource satelliteResource = GetSatelliteRxResource(technicalSystem.system.name);
			Function satelliteFunction = service.Functions.FirstOrDefault(x => x.Name.Equals("satellite", StringComparison.InvariantCultureIgnoreCase)) ?? throw new FunctionNotFoundException("satellite");
			satelliteFunction.Resource = satelliteResource;
			...
			// Assign Fixed Resource
			FixedEbuSatRxResourceHandler fixedResourceHandler = new FixedEbuSatRxResourceHandler(helpers);
			fixedResourceHandler.AssignFixedResource(service);

			Log(nameof(GenerateSatelliteRxService), $"Generated Sat RX service: {service.GetConfiguration().Serialize()}");
		...
		private FunctionResource GetSatelliteRxResource(string systemName)
		{
			...
			var resource = helpers.ResourceManager.GetResources(Net.Messages.ResourceExposers.PoolGUIDs.Contains(receptionSatelliteResourcePool.GUID)).FirstOrDefault(x => x.Name.StartsWith(satelliteName, StringComparison.InvariantCultureIgnoreCase));
		...
		private FunctionResource GetResource(string resourcePoolName, string resourceName)
		{
			...
			var resource = helpers.ResourceManager.GetResources(Net.Messages.ResourceExposers.PoolGUIDs.Contains(resourcePool.GUID)).FirstOrDefault(x => x.Name.Contains(resourceName));
		...
		private static void ConfigureSatelliteRxDemodulatingFunction(Function function, synopsisTechnicalSystem technicalSystem)
		{
			...
			ConfigureEncryptionTypeAndKeyProfileParameters(function, technicalSystem);
		}
		...
		private static void ConfigureSatelliteRxDecodingFunction(Function function, synopsisDestination destination, synopsisTechnicalSystem technicalSystem)
		{
			...
			ConfigureEncryptionTypeAndKeyProfileParameters(function, technicalSystem, destination);
		...
		private static void ConfigureEncryptionTypeAndKeyProfileParameters(Function function, synopsisTechnicalSystem technicalSystem, synopsisDestination destination = null)
		{
			// Encryption Key
		...
		private bool TryGenerateFullCapacityFiberRxService(synopsis synopsis, synopsisTechnicalSystem technicalSystem, out Service fullCapacityFiberRxService)
		{
			// Init Service
			fullCapacityFiberRxService = GenerateDefaultIntegrationService(FiberRxFullCapacityServiceDefinition, synopsis.StartDateTime, synopsis.EndDateTime);
			fullCapacityFiberRxService.EurovisionTransmissionNumber = synopsis.transmissionNo;
			fullCapacityFiberRxService.EurovisionTechnicalSystemId = technicalSystem.technicalSystemId;

			// Configure Source Function
			Function sourceFunction = fullCapacityFiberRxService.Functions.FirstOrDefault() ?? throw new FunctionNotFoundException();
			...
			// Configure Audio Channels
			ConfigureAudioChannels(fullCapacityFiberRxService.AudioChannelConfiguration, technicalSystem);

			return true;
		}

		private bool TryGenerateFixedLineEbuRxService(synopsis synopsis, synopsisTechnicalSystem technicalSystem, synopsisRouting routing, out Service fixedLineReception)
		{
			fixedLineReception = null;
			if (!TryGetFixedLineEbuRxLocationAndResource(routing, out string locationName, out FunctionResource resource))
			{
				Log(nameof(TryGenerateFixedLineEbuRxService), $"Unable to define location and function resource based on routing");
				return false;
			}

			// Init Service
			fixedLineReception = GenerateDefaultIntegrationService(FixedLineEbuRxServiceDefinition, synopsis.StartDateTime, synopsis.EndDateTime);
			fixedLineReception.EurovisionTransmissionNumber = synopsis.transmissionNo;
			fixedLineReception.EurovisionTechnicalSystemId = technicalSystem.technicalSystemId;

			// Configure Source Function
			Function sourceFunction = fixedLineReception.Functions.FirstOrDefault();
			...
			// Configure Audio Channels
			ConfigureAudioChannels(fixedLineReception.AudioChannelConfiguration, technicalSystem);
		...
		private List<Service> GenerateTransmissions(synopsis synopsis)
		{
			...
			{
				helpers.Log(nameof(EurovisionIntegration), nameof(GenerateTransmissions), $"Synopsis does not define a YLE Transmission Origin");
			...
			helpers.Log(nameof(EurovisionIntegration), nameof(GenerateTransmissions), $"Generated {transmissions.Count} possible Transmission Services: {String.Join(", ", transmissions.Select(x => x.Definition.Name))}");
		...
		private Service GenerateSatelliteTxService(synopsis synopsis, synopsisOrigin origin, synopsisTechnicalSystem technicalSystem)
		{
			...
			Function encodingFunction = service.Functions.FirstOrDefault(x => x.Name.Equals("generic encoding", StringComparison.InvariantCultureIgnoreCase)) ?? throw new FunctionNotFoundException("generic encoding");
			ConfigureSatelliteTxEncodingFunction(encodingFunction, technicalSystem);

			// Configure Modulating Function
			Function modulatingFunction = service.Functions.FirstOrDefault(x => x.Name.Equals("generic modulating", StringComparison.InvariantCultureIgnoreCase)) ?? throw new FunctionNotFoundException("generic modulating");
		...
		{
			ProfileParameter upLinkFrequencyProfileParameter = function.Parameters.FirstOrDefault(x => x.Name == SrmConfiguration.UplinkFrequencyProfileParameterName) ?? throw new ProfileParameterNotFoundException(SrmConfiguration.UplinkFrequencyProfileParameterName);
		...
		{
			ProfileParameter polarizationProfileParameter = function.Parameters.FirstOrDefault(x => x.Name == SrmConfiguration.PolarizationProfileParameterName) ?? throw new ProfileParameterNotFoundException(SrmConfiguration.PolarizationProfileParameterName);
		...
		private static void ConfigureSatelliteTxModulatingFunctionFec(Function function, StreamName streamName)
		{
			ProfileParameter fecProfileParameter = function.Parameters.FirstOrDefault(x => x.Name == SrmConfiguration.FecProfileParameterName);
			if (fecProfileParameter == null) throw new ProfileParameterNotFoundException(SrmConfiguration.FecProfileParameterName);
			if (!fecProfileParameter.Discreets.Exists(x => x.InternalValue.Equals(streamName.FEC)))
			{
				throw new InvalidProfileParameterValueException(fecProfileParameter.Name, streamName.FEC);
			}
		...
		private FunctionResource GetSatelliteTxResource(string systemName)
		{
			...
			var resource = helpers.ResourceManager.GetResources(Net.Messages.ResourceExposers.PoolGUIDs.Contains(transmissionSatelliteResourcePool.GUID)).FirstOrDefault(x => x.Name.StartsWith(systemName, StringComparison.InvariantCultureIgnoreCase));
		...
		private Service GenerateFullCapacityFiberTxService(synopsis synopsis, synopsisTechnicalSystem technicalSystem)
		{
			...
			Function destinationFunction = service.Functions.FirstOrDefault() ?? throw new FunctionNotFoundException();
		...
		private static void ConfigureAudioChannels(AudioChannelConfiguration audioChannelConfiguration, synopsisTechnicalSystem technicalSystem)
		{
			...
			for (int i = 0; i < technicalSystem.audios.Length; i++)
			{
				...
				AudioChannelPair audioChannelPair = audioChannelConfiguration.AudioChannelPairs.FirstOrDefault(x => x.Channel == pair) ?? throw new NotFoundException($"Unable to find audio channel pair with channel {pair}");
		...
		public Order GenerateEbuOrder(synopsis synopsis, out Exception exception)
		{
			try
			{
				exception = null;
				...
				// Generate Receptions
				List<CustomerService> customerServices = GenerateReceptions(synopsis);
				if (customerServices.Any() && customerServices.Any(cs => !cs.Company.Equals(customerServices[0].Company)))
				{
					Log(nameof(GenerateEbuOrder), $"A single synopsis cannot define receptions for multiple companies: {String.Join(", ", customerServices.Select(cs => cs.Service.Definition.VirtualPlatform + " -> " + cs.Company))}");
				}
				else
				{
					receptions = customerServices.Select(x => x.Service).ToList();
					receptionContract = customerServices.Any() ? customerServices[0].Contract : null;
				}

				// Generate Transmissions
				transmissions = GenerateTransmissions(synopsis);

				if (!receptions.Any() && !transmissions.Any())
				{
					helpers.Log(nameof(EurovisionIntegration), nameof(GenerateEbuOrder), $"Unable to generate any reception or transmission services based on the synopsis.");
			...
			{
				Log(nameof(GenerateEbuOrder), $"An issue has happened during creation of EBU order. {e}");
				exception = e;
				return null;
			}
		}
		...
		private Order GenerateOrder(synopsis synopsis, List<Service> receptions, Contract receptionContract, List<Service> transmissions, Contract transmissionContract)
		{
			...
			string orderName = DetermineOrderManualName(synopsis);

			// Determine security view ids
			var securityViewIds = GetSecurityViewIds(contract);

			// Build Order
			Order order = new Order
			{
				ManualName = orderName,
				NamePostFix = $" [{synopsis.transmissionNo}]",
			...
			order.SetSecurityViewIds(securityViewIds);
		...
		private HashSet<int> GetSecurityViewIds(Contract contract)
		{
			var viewIds = new HashSet<int>
			{
				...
				contract.CompanySecurityViewId
			};

			// If the company has linked orders, the EBU orders should be visible to them as well
			viewIds.UnionWith(contract.LinkedCompanies.Select(c => c.SecurityViewId));

			Log($"Security View IDs based on contract: '{string.Join(",", viewIds)}'");
		...
		private Note GenerateEbuNote(synopsis synopsis)
		{
			var note = new Note
			{
				Title = $"{DetermineOrderManualName(synopsis)} [{synopsis.transmissionNo}]",
		...
		private static string DetermineOrderManualName(synopsis synopsis)
		{
			var orderName = new StringBuilder();

			if (!string.IsNullOrWhiteSpace(synopsis.nature1))
			{
				orderName.Append(synopsis.nature1);
			}

			if (!string.IsNullOrWhiteSpace(synopsis.nature2))
			{
				if (orderName.Length > 0) orderName.Append(" ");
				orderName.Append(synopsis.nature2.Trim());
			}

			if (orderName.Length == 0)
			{
				orderName.Append("Eurovision");
			}

			return orderName.ToString().Clean();
		}
		...
		/// <param name="synopsis">Synopsis that was received from EBU.</param>
		private void UpdateOrder(Order newOrder, Order existingOrder, synopsis synopsis)
		{
			// Get Generated Services
			Service newReceptionService = newOrder.MainSourceService ?? throw new NotFoundException("Unable to find main source service"); // Never Null (Dummy or Reception)
			...
			foreach (Service existingService in existingServices)
			{
				switch (existingService.Definition.VirtualPlatform)
				{
					case VirtualPlatform.ReceptionEurovision:
						...
						existingOrder.ReplaceService(existingService, newReceptionService);
						break;
					case VirtualPlatform.TransmissionEurovision:
						// Eurovision Transmission that was configured from the CustomerUI
						// Replace with new service
						existingOrder.ReplaceService(existingService, newTransmissionService);
						break;
					case VirtualPlatform.ReceptionNone:
						...
						ReplaceReceptionNoneService(existingService, newReceptionService, existingOrder, newTransmissionService);
						break;
					default:
						...
						UpdateService(existingService, newReceptionService, existingOrder, newTransmissionService);
						break;
				}
			}
			...
			existingOrder.LateChange = true;

			if (existingOrder.CreatedByUserName == "Eurovision")
			{
				// only update order name based on synopsis if the order was originally created by Eurovision integration
				existingOrder.ManualName = newOrder.ManualName;
			}

			existingOrder.NamePostFix = newOrder.NamePostFix;

			// Optionally align service timing
			AlignServiceTimings(existingOrder, synopsis);
			bool orderIsOngoing = existingOrder.Start <= DateTime.Now && DateTime.Now <= existingOrder.End;

			if (orderIsOngoing && existingOrder.Status == Status.Preliminary)
			{
		...
		private static void ReplaceReceptionNoneService(Service existingService, Service newReceptionService, Order existingOrder, Service newTransmissionService)
		{
		...
		private void UpdateService(Service existingService, Service newReceptionService, Order existingOrder, Service newTransmissionService)
		{
		...
		private void UpdateService(Order existingOrder, Service newService, Service existingService)
		{
			...
			UpdateService(existingService, actualNewService, IgnoredItems.ServiceSelection | IgnoredItems.UnenforcedResources);
		}
		...
		private bool CanOrderBeBooked(Order order)
		{
			// Check for Destination, Recording or Transmission
			bool hasDestination = order.AllServices.Exists(x => x.Definition != null && x.Definition.VirtualPlatform == VirtualPlatform.Destination);
			bool hasRecording = order.AllServices.Exists(x => x.Definition != null && x.Definition.VirtualPlatform == VirtualPlatform.Recording);
			bool hasTransmission = order.AllServices.Exists(x => x.Definition != null && x.Definition.VirtualPlatformServiceType == VirtualPlatformType.Transmission);

			var allDummyServices = order.AllServices.Where(x => x.IsDummy).ToList();

			Log(nameof(CanOrderBeBooked), $"Dummy services: {String.Join(", ", allDummyServices.Select(x => x.Name))}");

			// Check if order has any Dummy Services
			bool hasDummy = allDummyServices.Any();
		...
		private void UpdateOrderStatus(Order order)
		{
			...
			else
			{
				...
				bool servicesChanged = changeSummary.AreThereAnyCrucialServiceChanges;

				if (nameChanged || timingChanged || servicesAddedOrRemoved || servicesChanged)
				{
		...
		private void AddOrUpdateEurovisionOrder(string transmissionNumber, Order order, synopsis synopsis)
		{
			...
			if (existingOrders.Any())
			{
				...
				{
					SendResponseToOrderManagerElement(order.EurovisionTransmissionNumber, UpdateStatus.OK, $"AddOrUpdateEurovisionOrder|No Update Required|EBU order end {order.End} is in the past (now: {DateTime.Now})");
					return;
				}
			...
			else
			{
				...
				{
					SendResponseToOrderManagerElement(order.EurovisionTransmissionNumber, UpdateStatus.OK, $"AddOrUpdateEurovisionOrder|No Update Required|EBU order start {order.Start} is in the past or too close to now ({DateTime.Now})");
					return;
				}
				...
				// New orders are always created in preliminary state
			...
			{
				SendResponseToOrderManagerElement(order.EurovisionTransmissionNumber, UpdateStatus.OK, "AddOrUpdateEurovisionOrder|Action succeeded|Adding or updating order succeeded", orderId, eventId);
			}
			else
			{
				SendResponseToOrderManagerElement(order.EurovisionTransmissionNumber, UpdateStatus.Failed, "AddOrUpdateEurovisionOrder|Action failed|Adding or updating order failed");
			}
		}
		...
		private List<Order> GetManuallyBookedOrders(string transmissionNumber)
		{
			...
			foreach (var element in elements)
			{
				...
				{
					new Skyline.DataMiner.Core.DataMinerSystem.Common.ColumnFilter { Pid = WorkOrdersTableTransmissionNumberPid, Value = transmissionNumber, ComparisonOperator = Core.DataMinerSystem.Common.ComparisonOperator.Equal }
				});
				...
				{
					new Skyline.DataMiner.Core.DataMinerSystem.Common.ColumnFilter { Pid = WorkOrdersTableNumberPid, Value = transmissionNumber, ComparisonOperator = Core.DataMinerSystem.Common.ComparisonOperator.Equal }
				});
			...
			{
				helpers.Log(nameof(EurovisionIntegration), nameof(GetManuallyBookedOrders), $"Found no linked Work Orders for Transmission Number: {transmissionNumber}");
			}
		...
		private void AddOrUpdateEurovisionNote(Note note, Exception ex = null)
		{
			...
			{
				SendResponseToOrderManagerElement(note.EurovisionId, UpdateStatus.OK, "AddOrUpdateEurovisionNote|Action succeeded|Adding or updating note succeeded | Exception: " + ex);
			}
			else
			{
				SendResponseToOrderManagerElement(note.EurovisionId, UpdateStatus.Failed, "AddOrUpdateEurovisionNote|Action failed|Adding or updating note failed | Exception: " + ex);
			}
		}
		...
		private bool AddOrUpdateOrder(Order order)
		{
			try
			{
				...
				if (!tasks.Exists(x => x.Status == TaskStatus.Fail)) return true;
		...
		{
			public CustomerService(Contract contract, Service service)
			{
			...
			public string Company => Contract.Company;
		}
	}
}