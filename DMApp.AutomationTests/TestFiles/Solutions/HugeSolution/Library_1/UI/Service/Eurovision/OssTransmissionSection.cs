namespace Skyline.DataMiner.DeveloperCommunityLibrary.YLE.UI.Service.Eurovision
{
	...
	public class OssTransmissionSection : Section
	{
		private readonly EurovisionBookingDetails details;
		private readonly OssTransmissionSectionConfiguration configuration;
		...
		private readonly DropDown facilityDropDown = new DropDown();
		...
		public OssTransmissionSection(EurovisionBookingDetails details, VirtualPlatformType virtualPlatformType, OssTransmissionSectionConfiguration configuration, Helpers helpers = null)
		{
		...
		{
			var organizations = details.GetOssTransmissionOrigins(virtualPlatformType);
		...
		{
			var selectedOrganization = details.GetOssTransmissionOrigins(virtualPlatformType).FirstOrDefault(x => x.Code.Equals(details.OriginOrganizationCode));
		...
		{
			var facilityOptions = details.Facilities.Any() ? details.Facilities.Select(f => f.ProductName).ToList() : new List<string> { "None" };
		...
		{
			var destinations = details.GetOssTransmissionDestinations(virtualPlatformType);
		...
		{
			var organization = details.GetOssTransmissionDestinations(virtualPlatformType).FirstOrDefault(x => x.Code.Equals(details.DestinationOrganizationCode));
