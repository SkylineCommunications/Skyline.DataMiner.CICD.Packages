	...
	public class Script : IDisposable
	{
		...
		public void Run(Engine engine)
		{
			try
			{
				...
				{
					helpers.NonLiveOrderManager.UnassignNonLiveOrder(nonLiveOrder, user);
				}
				else if (helpers.UserTaskManager.TryGetUserTask(scriptParameter, out var userTask))
				{
					userTask.Owner = String.Empty;
				...
				else
				{
					...
					ShowMessageDialog(engine, $"Unassigning the ticket failed because the system was unable to convert the ticket with ID {scriptParameter} to a non-live order or user task.", "Error");
				}
			}
		...
		{
			//engine.ShowUI();
			helpers = new Helpers(engine, Scripts.UnassignTicket);

			helpers.Log(nameof(Script), "START SCRIPT", "UnassignTicket");
		...
		private void ShowExceptionDialog(Engine engine, Exception exception)
		{
			...
			exceptionDialog.OkButton.Pressed += (o, e) => engine.ExitSuccess("Something went wrong during the execution of unassign ticket");
